...
> [!IMPORTANT]  
> This tap is still under development and is not recommended for use in its current form. <!-- TODO: remove disclaimer when feature-complete. -->
...
| parquet_partitioned           | False    |       0 | Whether to read from `file_path` as a partitioned data set or a series of independent files each with their own (potentially conflicting) schemas. |
